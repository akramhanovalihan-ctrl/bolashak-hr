...
import { users, units, employees as employeesTable } from './tables.js';
...
async function upsertEmployee(emp, unitId) {
  ...
  const existing = await query(`SELECT id FROM ${employeesTable} WHERE employee_number = $1`, [key]);
  ...
  if (existing.rows[0]) {
    await query(
      `UPDATE ${employeesTable} SET full_name=$1, unit_id=$2, position=$3, salary=$4,
  ...
  const id = randomUUID();
  await query(
    `INSERT INTO ${employeesTable} (id, employee_number, full_name, birth_date, unit_id, position,
...
export async function importOrgData() {
  ...
  for (const user of USERS) {
    if (user.unit_code && unitIds[user.unit_code]) {
      await query(`UPDATE ${users} SET unit_id = $1 WHERE email = $2`, [unitIds[user.unit_code], user.email]);
    }
  }

  for (const unit of UNITS) {
    if (unit.manager && unitIds[unit.code] && userIds[unit.manager.email]) {
      await query(`UPDATE ${users} SET unit_id = $1 WHERE email = $2`, [unitIds[unit.code], unit.manager.email]);
    }
  }
  ...
  if (source === 'org-chart') {
    await query(`UPDATE ${employeesTable} SET status = 'terminated' WHERE status = 'active'`);
  }
