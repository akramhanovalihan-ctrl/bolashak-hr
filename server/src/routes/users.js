...
router.get('/', requireAuth, requireRoles('admin', 'hr'), async (req, res) => {
  ...
  let sql = `
    SELECT u.id, u.email, u.full_name, u.role, u.unit_id, u.is_active, u.created_at,
...
router.patch('/:id', requireAuth, requireRoles('admin', 'hr'), async (req, res) => {
  const { is_active, role, unit_id, full_name } = req.body;
  ...
  const { rows: updated } = await query(
    `SELECT u.id, u.email, u.full_name, u.role, u.unit_id, u.is_active, u.created_at,
            un.name AS unit_name
     FROM ${users} u LEFT JOIN ${units} un ON un.id = u.unit_id WHERE u.id = $1`,
    [req.params.id]
  );
