...
async function requireAuth(ctx) {
  ...
  if (!c) {
    await ctx.reply('Сначала /start и привязка по ФИО или контакту.');
    return null;
  }
  return c;
}

export async function handleTabel(ctx) {
...
export async function handleZp(ctx) {
  const c = await requireAuth(ctx);
  if (!c) return;
...
export async function handleGrafik(ctx) {
  const c = await requireAuth(ctx);
  if (!c) return;
...
export async function handleOtsutstvie(ctx) {
  const c = await requireAuth(ctx);
  if (!c) return;
...
export async function handleMoiZayavki(ctx) {
  const c = await requireAuth(ctx);
  if (!c) return;
...
export function registerEmployee(bot) {
  cmd(bot, ['tabel', 'tabl'], handleTabel);
  cmd(bot, 'zp', handleZp);
  cmd(bot, ['grafik', 'grafic', 'график'], handleGrafik);
  cmd(bot, ['otsutstvie', 'otsustive', 'otsutstvye', 'отсутствие'], handleOtsutstvie);
  cmd(bot, ['moi_zayavki', 'zayavki'], handleMoiZayavki);
